...
- **the picker's `'soon'` state is unreachable — and the branch stays.** `ALL_SECTION_TYPES` minus `RENDERABLE_SECTION_TYPES` is empty after 2C.2b, so `typeState` can no longer return `'soon'`. `test/render/sectionTypeCoverage.test.mjs` asserts that difference is empty and goes red *naming the type* the moment a declared type ships without a component — at which point the branch is live again, not obsolete. This one retires **backwards** from the others: a red here is not "delete the filter", it is "the filter is doing its job again". The branch is the only thing keeping a schema-only type disabled rather than clickable into a page that publishes empty, and the same file source-scans that it is still there.
...
| 2C.2b | ~~Data-backed, derived/time-varying components~~ **SHIPPED** (`course_list:source='skill'|'program'`, `course_schedule`) | Landed as the labelled exception §2C.2 argued for, not as a waiver of it: the canvas shows an edit-time **sample** and *says so*, in a neutral info tone at the field (`SectionContentEditor` / `fields.jsx`) — "ตัวอย่าง ณ เวลาแก้ไข", deliberately not the amber/red "not found" tone, because an author must never read *this is a sample* as *this is broken*. Both notes can show at once. **The two claims this row used to make are now false and are corrected here rather than left to rot:** `course_schedule` is in `REGISTRY`, and `course_list.source` is `z.enum(['manual','skill','program'])` — the narrowing to `['manual']` was the gate for exactly this phase and it was released when the phase landed, so the editor's three source options are all honoured. **The picker therefore shows no "เร็ว ๆ นี้" at all any more** — every declared type has a component. That is measured, not assumed, by `test/render/sectionTypeCoverage.test.mjs`, and the picker's `'soon'` branch is KEPT as the fail-closed path (see *Self-retiring assertions*). |
...
- **`updatePageStatus` now has NO live caller.** Its name says "set the status", but it has three times needed to know more: Item 7 found the editor must NOT use it (it sets status alone → publishes stale content and invalidates the conflict token — the editor publishes via a full save instead); the readiness gate made it read `slug`/`title`/`sections` to judge the doc it only stamps a status on; and the draft/published split found the third — it writes a bare `$set: { status }` with **no conflict check**, and on publish it snapshots `doc.toObject()`, which since round 1 carries `draft`. It would archive an unpublished edit as though it had been live while publishing the stale content it did not promote — wrong in both directions at once. **Round 3 retired its last caller**, the admin list's publish/unpublish toggle, which now uses `publishPageStatus`. The function stays, unmodified, as a narrow primitive; the next caller who reaches for it because the name sounds right needs all three lessons. Prefer `publishPageStatus` from anything changing status, and `updatePageBuilderPage` from anything holding a working tree.
...
3. **Component library missing.** Card/Dynamic/Advanced types were absent from the picker, so an author couldn't tell they exist. Fixed: all types display; a type is clickable only when it is in `REGISTRY` and tier-allowed. Displaying a type is not offering it. *(2C update: the 3 self-contained Cards + 4 Advanced now render, so they became clickable automatically — the picker derives from `REGISTRY`, so it needed no edit. Advanced are clickable for developers only.)* *(2C.2 update: the last 6 — the data-backed types — render too, so **nothing in the picker is labelled "เร็ว ๆ นี้" any more**. `typeState`'s `'soon'` branch is now unreachable, and it STAYS: it is the only thing that keeps a schema-only type disabled instead of clickable, and every type in this codebase has spent a phase in exactly that state. Unreachability is a measurement rather than a claim — `test/render/sectionTypeCoverage.test.mjs` subtracts the registry from the schema's type list and goes red, naming the type, the day the gap reopens.)*
...
1. **Back / Back-then-Forward across the create→edit `replaceState` — now GUARDED, and the guard is the thing that needs the click-test.** It was verified in the browser that Back left the editor with *no warning and no autosave flush*: `beforeunload` covers tab close and reload only, and App Router soft navigation fires none. Cost: autosave is a 5s idle debounce, and it never runs at all for an unsaved `/builder/new`, so Back on a new page destroyed the whole draft. Closed by `lib/pageBuilder/leaveGuard.js` (the pure decision, unit-tested exhaustively) plus `useLeaveGuard.js`, which wires **three** exits to it — `beforeunload`, `popstate`, and a capture-phase click on in-app anchors so the admin sidebar is covered.
   **What remains unverified is now the guard itself, and it cannot be otherwise:** jsdom has no session history, and a fake one would be the thing under test, so `test/fs/pageBuilderLeaveGuard.test.mjs` pins the *wiring* only and says so at the top. Needs a human pass, on a saved page **and** on `/builder/new`: (a) Back → confirm appears, cancel keeps the work; (b) Back → confirm → leave lands where Back was aiming; (c) Back-then-Forward — re-pushing the sentinel **discards forward entries**, so Forward no longer returns across this editor, which is pushState's own semantics and not a choice; (d) a sidebar link asks; (e) a link *inside the canvas* still selects its section and does **not** ask; (f) tab close and reload still show the browser's own prompt.
