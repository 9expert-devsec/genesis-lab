...
The 16 IGNORED cells are two controls, and both are in the _universal envelope_
...
in kind: _read, but only when a second field is set_. Every PARTIAL cell below
names its condition.
...
under-counts produces _false IGNORED findings_ — the first pass looked like it
...
- **Content is populated for every type.** Every 2C component fails closed and
  ...
  difference for _any_ control, and the whole matrix would read IGNORED.
...
`content.courseId` while holding `data` constant correctly reports _the renderer
does not read it_ — and would be misread as IGNORED. Those cells are marked
...
`button` style.buttonStyle · `ratio`/`cols`/`mob` layout._ · `id`/`class`/`css`/
`html` advanced._
...
_do_ read it. Real effect, entirely dependent on what is nested inside.
...
All 49 content controls are read. Split by _who_ reads them:
...
| full_width, container, two_column, card_grid, highlight_grid | _none_                                                                     | the tree edits their children                                          |
...
asserts an entry _exists_, never that it is empty. The self-retiring mechanism
...
| any _other_ offered value similarly inert?             | **No.** All five non-default values render differently from `default`. `default` is `''` legitimately — it inherits the theme surface |
...
`instructor_card`.** _2 cells, IGNORED._ Both components clamp themselves to
...
_Options for a fix: drop the self-clamp and let the envelope decide; or make the
control type-aware the way `SECTION_STYLE_CAPS` already makes the style props._

**2. `สีเน้น` (accentColor) does nothing on 14 of 27 types.** _14 cells,
IGNORED._ Nine components paint with it — eight name `--pb-accent-*` in their
...
_Counting note, because it bit this audit._ A first pass put the direct-consumer
...
**4. `Custom CSS` is silently inert without a valid `Section ID`.** _27 cells,
PARTIAL._ Both `advanced.customCss` and the whole `custom_css` type. Mitigated:
the panel warns at the field in both places, and the renderer logs in dev. Listed
here because it is the most _frequently encountered_ conditional in the panel,
not because it is unlabelled.

**5. The editor canvas cannot show responsive behaviour, and inverts
`visibility`.** _Not a matrix cell — see §7._ A `mobile_only` section vanishes in
...
**6. `accordion` does not take the section accent; `tabs` does.** _1 cell,
IGNORED (inside finding 2)._ Two sibling item-based types built in the same
...
does.** _§6._ `text-3xl` vs `text-3xl md:text-4xl` — same weight, same size, same
card family. Whichever is right, they should agree.

**8. `presets.js` claims a "loader check" that does not exist.** _§4._ The
...
_Not a matrix cell._ `ImageSection` reads only `src`; the media picker writes
...
`IGNORED` is defined in §1 as _offered, and no reader_. Both controls are still
...
3. **It goes stale silently, in the worst direction.** Finding 1's own suggested
   ...
   nothing that reddens. A stale _hint_ only reads wrong.
...
icon _looked_ accented, so an author who picked a different accent had no
...
accents a hover _state_ (`icon_card`'s tinted chip is a resting background). It
...
and JSDOM resolves none of that. What was _not_ run is a **layout** measurement:
...
directs, for an ADDED type: update finding 2 here, and delete the test _once the
offered set and the reader set agree_. They do not yet — two types remain — so
...
pattern _permits_ an accented border, but that is a decorative left-hand rule on
...
So the tripwire changes character instead of retiring: from _watch this set grow
toward agreement_ to _this set is closed — nothing joins or leaves without a
decision_. A twelfth direct consumer is now a claim that a type has an accent
...
container width changes cell _size_, not cell _count_:
...
message set the condition: _"if the clamp is GONE, re-run
scripts/\_probe-container-width.mjs and confirm the four settings now give four
distinct widths, then delete this test"_. Both halves were done, and the test is
...
`container` _did_ carry a width clamp, using it to show the `max-w-sm` scan was
...
override _replaces_ rather than layers; that the two types stay distinguishable;
...
one preset lookup — so it proves which _value_ each type starts at, never a
...
control _and_, measured, collapsed the widths straight back to
640 / 768 / 768 / 768.
...
that the pattern _permits_ an accented border — a decorative left-hand rule on a
...
So the tripwire changes character instead of retiring: from _watch this set grow
toward agreement_ to _this set is closed — nothing joins or leaves without a
decision_. An eleventh direct consumer is now a claim that a type has an accent
surface after all, which is an argument that should not be settled silently. Its
