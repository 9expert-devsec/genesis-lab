 ...
 * The charge route sets this as return_uri with ?registrationId=xxx; we poll
 * /api/registration/public/status until the charge settles.
 *
 * The status logic lives in an inner component wrapped in <Suspense> because
 * useSearchParams() requires a suspense boundary in the App Router (a bare
 * top-level call fails `next build`).
 *
 * No conversion tracking here — unlike the masterclass flow, the public
 * registration wizard does not fire analytics on its paid paths, so there is
    ...
    const timer = setInterval(async () => {
      ...
      try {
        const res = await fetch(
          `/api/registration/public/status?id=${encodeURIComponent(registrationId)}`,
          { cache: 'no-store' }
        );
...
export default function RegistrationPaymentCompletePage() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen flex flex-col items-center justify-center gap-4 text-center px-4">
