...
export function AddFeaturedOnlineCourseForm({ courses = [] }) {
  ...
  function handleAdd() {
    ...
    startTransition(async () => {
      const result = await addFeaturedOnlineCourse(fd);
      if (result.ok) {
        setMessage({
          type: 'ok',
  ...
  return (
    <div className="space-y-3" ref={wrapRef}>
      ...
      <div className="relative">
        ...
        >
          ...
          <input
            ...
            placeholder="พิมพ์ชื่อคอร์สหรือ Course ID เช่น Power Query, ONL-MSE"
      ...
      {selected && (
        <div className="flex items-center gap-3 rounded-9e-md border border-9e-sky/30 bg-9e-ice p-3">
          ...
          <button
            ...
            onClick={handleAdd}
            disabled={isPending}
            className="flex shrink-0 items-center gap-1.5 rounded-9e-md bg-9e-primary px-4 py-2 text-sm font-bold text-white transition-colors hover:bg-9e-brand disabled:opacity-50"
          >
            {isPending ? (
