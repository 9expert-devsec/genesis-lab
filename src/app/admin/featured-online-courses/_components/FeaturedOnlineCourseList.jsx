...
import {
  deleteFeaturedOnlineCourse,
  updateFeaturedOnlineCourse,
...
export function FeaturedOnlineCourseList({ courses: initial }) {
  async function persistReorder(newOrder, prevOrder) {
    ...
    const updates = newOrder
      .map((c, newIdx) => {
        ...
        return updateFeaturedOnlineCourse(c._id, fd);
      })
      .filter(Boolean);
  ...
  async function handleDelete(id) {
    ...
    startTransition(async () => {
      await deleteFeaturedOnlineCourse(id);
  ...
  async function handleToggle(course) {
    ...
    startTransition(async () => {
      await updateFeaturedOnlineCourse(course._id, fd);
      setCourses(
        courses.map((c) =>
  ...
  async function handleReorder(course, direction) {
    ...
    startTransition(async () => {
      await Promise.all([
        updateFeaturedOnlineCourse(a._id, fdA),
        updateFeaturedOnlineCourse(b._id, fdB),
      ]);
      const next = [...courses];
