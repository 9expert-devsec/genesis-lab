...
import {
  deleteNavFeaturedOnlineCourse,
  updateNavFeaturedOnlineCourse,
} from '@/lib/actions/nav-featured-online-courses';
import { useDragReorder } from '@/hooks/useDragReorder';
import { DragHandle } from '@/components/ui/DragHandle';

export function NavFeaturedOnlineCourseList({ courses: initial }) {
  async function persistReorder(newOrder, prevOrder) {
    ...
    const updates = newOrder
      .map((c, newIdx) => {
        ...
        return updateNavFeaturedOnlineCourse(c._id, fd);
      })
      .filter(Boolean);
  ...
  async function handleDelete(id) {
    if (!confirm('ลบออกจากรายการ?')) return;
    setBusyId(id);
    startTransition(async () => {
      await deleteNavFeaturedOnlineCourse(id);
  ...
  function handleToggle(course) {
    ...
    startTransition(async () => {
      const result = await updateNavFeaturedOnlineCourse(course._id, fd);
      if (result?.ok === false) {
        // Hit the 3-active cap — leave state unchanged, surface the reason.
        alert(result.error ?? 'ไม่สามารถเปิดใช้งานได้');
        setBusyId(null);
        return;
      }
      setCourses(
  ...
  async function handleReorder(course, direction) {
    ...
    startTransition(async () => {
      await Promise.all([
        updateNavFeaturedOnlineCourse(a._id, fdA),
        updateNavFeaturedOnlineCourse(b._id, fdB),
      ]);
      const next = [...courses];
          ...
          {courses.length === 0 && (
            <tr>
              <td colSpan={5} className="py-8 text-center text-9e-slate-dp-50">
                ยังไม่มีคอร์ส — เพิ่มด้วยช่องค้นหาด้านบน
              </td>
            </tr>
          )}
          {courses.map((c, i) => {
