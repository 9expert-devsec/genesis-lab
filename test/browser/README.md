...
| `prose-lists.mjs` | Tiptap's `<li><p>…</p></li>` lists copy as **one line per item** (no blank line), a two-paragraph item and a nested list keep their breaks, the gap between items is unchanged in all eight public rich-text wrappers (16/16/4/16/20/24/24/6 px, measured before the rule existed), and the admin editor's `.ProseMirror` surface is untouched. Drives a `data:` page carrying the **built** stylesheet, so it needs `npm run build` and NOT the dev server — `node test/browser/prose-lists.mjs` runs it alone. |
