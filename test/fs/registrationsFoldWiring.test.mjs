...
test("all three query actions group by the SHARED key expression", () => {
  ...
  for (const [name, body] of [
    ...
    ["getRegistrationTotal", TOTAL],
    ["getRegistrationStatusCounts", COUNTS],
  ]) {
    assert.match(
      body,
      /_id:\s*REQUEST_KEY_EXPR/,
...
test("no action rolls its own grouping key", () => {
  ...
  for (const [name, body] of [
    ...
    assert.ok(
      !/bundle\.requestId'\s*\}\s*\}/.test(body.replace(/\s+/g, " ")) ||
        /REQUEST_KEY_EXPR/.test(body),
...
test("CONTROL: the body extractor found three real functions", () => {
  ...
  assert.ok(
    LIST.includes("PAGE_SIZE"),
    "the listRegistrations body is not the list",
  );
  assert.ok(
    TOTAL.includes("countDocuments"),
    "the getRegistrationTotal body is not the total",
  );
  assert.ok(
    COUNTS.includes("INHOUSE_STATUS_VALUES"),
    "the counts body is not the counts",
  );
  for (const body of [LIST, TOTAL, COUNTS])
    assert.ok(body.length > 300, `a body parsed to ${body.length} chars`);
});
...
test("$skip and $limit come AFTER $group — pagination is over REQUESTS", () => {
  ...
  assert.notEqual(
    group,
    -1,
    "no $group in listRegistrations — the fold is gone",
  );
  ...
  assert.ok(
    skip < limit,
    "$limit precedes $skip — the page window is inverted",
  );
});
...
test("the pagination property, reported by LINE so a reviewer can look", () => {
   ...
   * several.
   */
  const rawStart = ACTIONS.raw.indexOf(
  ...
  const lineOf = (needle) => {
    ...
    const hits = [];
    lines.forEach((l, i) => {
      if (l.includes(needle)) hits.push(baseLine + i);
    });
    assert.equal(
      hits.length,
      1,
    ...
    assert.ok(
      ...
      `computed line for "${needle}" is not a positive integer: ${n}`,
    );
    return n;
  };
  ...
  console.log(
    `      [fold] $group at line ${groupLine}, $skip at ${skipLine}, $limit at ${limitLine}`,
  );
  assert.ok(
    groupLine < skipLine && skipLine < limitLine,
...
test("the count of requests is its own grouped pipeline, not countDocuments", () => {
  ...
  assert.match(
    LIST,
    /\$count:\s*'n'/,
    "the public total is not a grouped $count",
  );
  assert.ok(
    !/const total = await Model\.countDocuments\(filter\)/.test(LIST),
...
test("the re-fetch covers BOTH kinds of key", () => {
  ...
  assert.match(
    LIST,
    /\{ _id: \{ \$in: ids \} \}/,
    "plain registrations are not fetched back",
  );
  assert.match(
    LIST,
    /\{ 'bundle\.requestId': \{ \$in: keys \} \}/,
    "bundle siblings are not fetched back",
  );
});
...
test("the dashboard does not restate either rule as a second copy", () => {
   ...
   */
  assert.equal(
    /\$ifNull:\s*\[\s*'\$bundle\.requestId'/.test(BUILD_METRICS.code),
    false,
    "buildMetrics writes its own request-key expression instead of importing REQUEST_KEY_EXPR",
  );
  assert.equal(
    /REQUEST_STATUS_PRECEDENCE|\$setIsSubset/.test(BUILD_METRICS.code),
    false,
...
test("CONTROL: the copy-detector finds a planted second definition", () => {
  ...
  const planted =
    "reqKey: { $ifNull: ['$bundle.requestId', { $toString: '$_id' }] },";
  assert.ok(
    /\$ifNull:\s*\[\s*'\$bundle\.requestId'/.test(planted),
    "the key-copy matcher does not recognise the very shape it forbids",
  );
  assert.ok(
    /REQUEST_STATUS_PRECEDENCE|\$setIsSubset/.test(
      "case: { $setIsSubset: [f, T] }",
    ),
  ...
  assert.ok(
    BUILD_METRICS.code.length > 2000,
...
test("the seat count that KEEPS legs says so where it is rendered", () => {
  ...
  const panel = readSource(
    "src/app/admin/schedules/_components/RegistrationSummaryPanel.jsx",
  );
  assert.match(
    panel.code,
    /ผู้เข้าอบรมในรอบนี้/,
...
test("CONTROL: the note bound is the array’s own docblock, not the file", () => {
  ...
  assert.ok(
    note.length > 400 && note.length < 4000,
    `the note slice is ${note.length} chars`,
  );
  ...
  assert.equal(
    note.includes("WHY THIS IS GENERATED AND NOT WRITTEN OUT"),
    false,
