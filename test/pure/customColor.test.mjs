...
import {
  HEX_COLOR_RE, isHexColor, hexOrNull, COLOR_MODES,
  GRADIENT_DIRECTIONS, DEFAULT_GRADIENT_DIRECTION,
  customBackgroundStyle, hasCustomBackground,
  THEME_TEXT_RGB, contrastRatio, CONTRAST_MIN,
  backgroundContrastOk, accentContrastOk,
} from '@/lib/pageBuilder/customColor';
// ADDED beside the statement above rather than folded into it — the standing
// rule in this repo.
import {
  backgroundClass, isDarkBackground, accentVars,
  backgroundClassFor, backgroundStyleFor, isDarkBackgroundFor, accentVarsFor,
} from '@/lib/pageBuilder/presets';
import { BACKGROUNDS, ACCENTS, settingsSchema, styleSchema } from '@/lib/schemas/sections/base';
import { DRAFT_CONTENT_KEYS, LIVE_ONLY_KEYS, pageBuilderSchema } from '@/lib/schemas/pageBuilder';
...
const REFUSED = Object.freeze({
  ...
  '#0d1b2aff':       'eight-digit alpha — a colour nobody can predict over an unknown surface',
  ...
  'var(--9e-navy)':  'a token reference — legitimate elsewhere, not an author colour',
});

test('the accepted forms are exactly six digits, either case', () => {
  for (const v of ACCEPTED) {
    assert.equal(isHexColor(v), true, `${v} is no longer accepted`);
    assert.equal(hexOrNull(v), v, 'an accepted value was rewritten rather than passed through');
  }
});
...
test('every refused form is refused, and each names why', () => {
  for (const [value, why] of Object.entries(REFUSED)) {
    ...
    assert.equal(hexOrNull(value), null, `"${value}" did not fall back to null`);
  }
  // Non-strings too — a directly-seeded document can carry anything.
  for (const v of [null, undefined, 42, {}, [], true]) assert.equal(hexOrNull(v), null);
});
...
test('the SCHEMA refuses the same set the render layer refuses', () => {
  ...
  for (const value of Object.keys(REFUSED)) {
    assert.equal(styleSchema.safeParse({ accentMode: 'custom', accentCustom: value }).success, false,
      `the schema now stores "${value}" as an accent`);
    assert.equal(settingsSchema.safeParse({ backgroundCustom: { from: value } }).success, false,
      `the schema now stores "${value}" as a background stop`);
  }
  for (const value of ACCEPTED) {
    assert.equal(styleSchema.safeParse({ accentMode: 'custom', accentCustom: value }).success, true);
  }
});
...
test('an INVALID stored value renders the default, never a broken style', () => {
  ...
  for (const value of Object.keys(REFUSED)) {
    assert.equal(backgroundStyleFor({ backgroundMode: 'custom', backgroundCustom: { from: value } }), undefined,
      `"${value}" reached a style attribute`);
    // …and the preset class comes back, so the section is not left unpainted.
    assert.equal(
      backgroundClassFor({ backgroundMode: 'custom', background: 'dark', backgroundCustom: { from: value } }),
      backgroundClass('dark'));
    assert.equal(accentVarsFor({ accentMode: 'custom', accentCustom: value }), undefined);
  }
});
...
test('the schema accepts EMPTY for the second stop, and nothing else non-hex', () => {
  ...
  assert.equal(settingsSchema.safeParse({ backgroundCustom: { from: '#000000', to: '' } }).success, true);
  assert.equal(settingsSchema.safeParse({ backgroundCustom: { from: '#000000', to: '#abc' } }).success, false);
});
...
test('ONE stop emits a flat colour, not a gradient with two identical stops', () => {
  assert.deepEqual(customBackgroundStyle({ from: '#0d1b2a' }), { backgroundColor: '#0d1b2a' });
  assert.deepEqual(customBackgroundStyle({ from: '#0d1b2a', to: '' }), { backgroundColor: '#0d1b2a' });
  ...
  assert.deepEqual(customBackgroundStyle({ from: '#0d1b2a', to: '', direction: 'to_right' }),
...
test('every direction emits its own CSS, and an unknown one falls back', () => {
  const emitted = GRADIENT_DIRECTIONS.map(
    (d) => customBackgroundStyle({ from: '#000000', to: '#ffffff', direction: d }).backgroundImage);
  ...
  assert.equal(new Set(emitted).size, emitted.length, 'two directions emit the same CSS');
  // A direction the enum does not know resolves to the default rather than
  // producing `linear-gradient(undefined, …)`.
  assert.equal(
    customBackgroundStyle({ from: '#000000', to: '#ffffff', direction: 'sideways' }).backgroundImage,
    customBackgroundStyle({ from: '#000000', to: '#ffffff', direction: DEFAULT_GRADIENT_DIRECTION }).backgroundImage);
});
...
test('every stored preset value resolves EXACTLY as it did before this round', () => {
  ...
  for (const bg of BACKGROUNDS) {
    ...
    assert.equal(backgroundClassFor(settings), backgroundClass(bg), `background "${bg}" moved`);
    assert.equal(isDarkBackgroundFor(settings), isDarkBackground(bg), `darkness of "${bg}" moved`);
    assert.equal(backgroundStyleFor(settings), undefined, `background "${bg}" gained an inline style`);
  }
  for (const a of ACCENTS) {
    assert.deepEqual(accentVarsFor({ accentColor: a }), accentVars(a), `accent "${a}" moved`);
  }
...
test('the four values the corpus ACTUALLY stores are covered by the sweep above', () => {
  ...
  // nobody has.
  for (const bg of STORED_BACKGROUNDS) assert.ok(BACKGROUNDS.includes(bg), `${bg} left the enum`);
  for (const a of STORED_ACCENTS) assert.ok(ACCENTS.includes(a), `${a} left the enum`);
});
...
test('CONTROL: the comparison WOULD notice the preset path moving', () => {
  ...
  assert.throws(() => assert.equal(backgroundClass('dark'), backgroundClass('white')),
...
test('a stored section gains NOTHING when it is re-validated', () => {
  ...
  assert.deepEqual(Object.keys(styleSchema.parse({ accentColor: 'green' })), ['accentColor']);
  ...
  assert.deepEqual(Object.keys(settingsSchema.parse(undefined)).sort(),
    ['background', 'containerWidth', 'spacingBottom', 'spacingTop', 'visibility']);
...
test('the page-level DRAFT/LIVE partition is unmoved', () => {
  ...
  assert.deepEqual([...DRAFT_CONTENT_KEYS, ...LIVE_ONLY_KEYS].sort(), [...all].sort());
  assert.equal(DRAFT_CONTENT_KEYS.filter((k) => LIVE_ONLY_KEYS.includes(k)).length, 0);
});
...
test('a PRESET colour resolves through a var; a CUSTOM one is a literal', () => {
  ...
  for (const a of ACCENTS) {
    for (const v of Object.values(accentVars(a))) {
      assert.match(v, /^var\(--/, `preset accent "${a}" resolved to something other than a token`);
    }
  }
  const custom = accentVarsFor({ accentMode: 'custom', accentCustom: '#123456' });
  assert.equal(custom['--pb-accent-fill'], '#123456');
  assert.equal(custom['--pb-accent-text'], '#123456', 'a custom accent was degraded — the author chose it');
...
test('--pb-accent-on flips with the accent luminance, and only that one var', () => {
  ...
  const light = accentVarsFor({ accentMode: 'custom', accentCustom: '#ffffff' });
  assert.notEqual(dark['--pb-accent-on'], light['--pb-accent-on'],
...
test('a custom BACKGROUND never asks for light text — the theme keeps that', () => {
  ...
  for (const from of ['#000000', '#ffffff', '#0d1b2a']) {
    assert.equal(isDarkBackgroundFor({ backgroundMode: 'custom', backgroundCustom: { from } }), false,
...
test('hasCustomBackground needs BOTH the mode and a usable colour', () => {
  assert.equal(hasCustomBackground({ backgroundCustom: { from: '#000000' } }), false, 'mode not chosen');
  assert.equal(hasCustomBackground({ backgroundMode: 'custom' }), false, 'no colour');
  assert.equal(hasCustomBackground({ backgroundMode: 'preset', backgroundCustom: { from: '#000000' } }), false);
  assert.equal(hasCustomBackground({ backgroundMode: 'custom', backgroundCustom: { from: '#000000' } }), true);
...
test('the theme text triples still equal the tokens they mirror', () => {
  ...
  const tokenRgb = (name) => {
    ...
    assert.ok(m, `tailwind.config.js no longer declares a 6-digit hex for "${name}"`);
...
test('the accent warning fires on a colour too pale to read as text', () => {
  assert.equal(accentContrastOk('#ffff00'), false, 'yellow passed as body text on a light page');
  assert.equal(accentContrastOk('#0d1b2a'), true);
  assert.equal(accentContrastOk(''), true, 'an empty value warned about nothing');
});
...
test('CONTROL: the contrast helper discriminates — it is not a constant', () => {
  ...
  const answers = ['#ffffff', '#808080', '#000000'].map((from) => backgroundContrastOk({ from }));
...
test('no round-39 SOURCE file carries a hex literal', () => {
  ...
  for (const rel of [
    ...
    assert.deepEqual(rawColors(readSource(rel).code), [],
