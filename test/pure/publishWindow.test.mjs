...
import {
  isPubliclyVisible,
  invisibleReason,
} from "@/lib/pageBuilder/visibility";
...
test("the conversion does not depend on the machine that runs it", () => {
  ...
  assert.equal(
    seen.size,
    1,
    `the window moved with the runtime zone: ${[...seen].join(" vs ")}`,
  );
...
test("CONTROL: the OLD conversion really did move with the zone", () => {
  ...
  assert.ok(
    seen.has("2026-08-27T17:00:00.000Z"),
    "the Bangkok reading is missing",
  );
...
test("the zone is IMPORTED from the module that already owned it", () => {
  ...
  const { code, withImports } = readSource(
    "src/lib/pageBuilder/publishWindow.js",
  );
  assert.match(
    withImports,
...
test("CONTROL: the literal matcher does see a restated zone", () => {
  assert.equal(
    "const OFF = '+07:00';".includes("'+07:00'"),
    true,
...
test("a page whose window ends TODAY is visible all of today", async (t) => {
  await t.test("visible at 00:00:00.000 Bangkok on that day", () => {
    assert.equal(
      isPubliclyVisible(endsToday, at(DAY_START_UTC)),
      true,
  ...
  await t.test(
    "visible at 23:59:59.999 Bangkok — the LAST visible instant",
    () => {
  ...
  await t.test(
    "NOT visible at 00:00:00.000 Bangkok the next day — the FIRST hidden one",
    () => {
  ...
  await t.test(
    "the two boundary instants are 1ms apart, and they straddle Bangkok midnight",
    () => {
  ...
  await t.test("CONTROL: the OLD conversion fails the very first case", () => {
    ...
    const oldStyle = {
      status: "published",
      publishStartDate: null,
      publishEndDate: DAY_START_UTC,
    };
    assert.equal(
      isPubliclyVisible(oldStyle, at(DAY_START_UTC)),
      true,
      "precondition: visible AT the instant",
    );
    assert.equal(
      isPubliclyVisible(oldStyle, at(DAY_START_UTC) + 1),
      false,
...
test("a START date opens the window at the start of its day", () => {
  ...
  assert.equal(
    isPubliclyVisible(startsToday, at(DAY_START_UTC)),
    true,
    "not live at 00:00 on its own start day",
  );
  assert.equal(
    isPubliclyVisible(startsToday, at(DAY_START_UTC) - 1),
    false,
    "live one ms before it begins",
  );
  assert.equal(
    invisibleReason(startsToday, at(DAY_START_UTC) - 1),
    "published_future",
  );
});
...
test("a value round-trips through the box without walking", () => {
  ...
  assert.equal(
    ...
    LAST_VISIBLE_UTC,
    "the end date drifted across five round trips",
  );

  let start = windowStartFromInput(DAY);
  for (let i = 0; i < 5; i += 1)
    start = windowStartFromInput(toDateInput(start));
  assert.equal(
    start,
    DAY_START_UTC,
    "the start date drifted across five round trips",
  );
});
...
test("CONTROL: the OLD pair DID walk backwards, one day per save", () => {
  ...
  withTZ("Asia/Bangkok", () => {
    ...
    assert.equal(
      oldTo(v),
      "2026-08-27",
      "precondition: the box already reads a day early",
    );
    v = OLD_CONVERSION(oldTo(v));
    assert.equal(
      oldTo(v),
      "2026-08-26",
      "the old pair did not walk — the fixture proves nothing",
    );
  });
});
...
test("an empty or unparseable box is NO bound, never a bound matching nothing", () => {
  for (const bad of [
    ...
    assert.equal(
      windowStartFromInput(bad),
      null,
      `start accepted ${JSON.stringify(bad)}`,
    );
    assert.equal(
      windowEndFromInput(bad),
      null,
      `end accepted ${JSON.stringify(bad)}`,
    );
  }
...
test("a calendar date that does not exist is REFUSED, not rolled forward", () => {
  ...
  for (const impossible of [
    ...
    assert.equal(
      windowEndFromInput(impossible),
      null,
      `${impossible} was accepted`,
    );
    assert.equal(
      windowStartFromInput(impossible),
      null,
      `${impossible} was accepted`,
    );
  }
...
test("CONTROL: the extractor really does read the function, and would see an edit", () => {
  ...
  assert.ok(
    src.includes("now > end"),
    "the extractor is not reading the comparison",
  );
...
const EXPIRED_REASON_TEXT =
  "เลยวันสิ้นสุดแล้ว — หน้านี้เข้าไม่ได้ แม้สถานะจะเป็นเผยแพร่";

const honestyLine = (next, now) =>
  isPubliclyVisible(next, now)
    ? VISIBLE_LINE
...
test("an end date of TODAY now reads as visible, and the route agrees", () => {
  ...
  assert.equal(
    isPubliclyVisible(next, noon),
    true,
    "the route would 404 a page ending today",
  );
...
test("the dialog builds its window from these conversions and nothing else", () => {
  const { code, withImports } = readSource(
    "src/components/pageBuilder/editor/PublishDialog.jsx",
  );
  assert.match(
    withImports,
  ...
  assert.match(
    ...
    "the start is not converted",
  );
  assert.match(
    code,
    /publishEndDate: windowEndFromInput\(end\)/,
    "the end is not converted",
  );
  assert.match(
    code,
    /useState\(toDateInput\(page\?\.publishStartDate\)\)/,
    "the start box is not read in Bangkok",
  );
  assert.match(
    code,
    /useState\(toDateInput\(page\?\.publishEndDate\)\)/,
    "the end box is not read in Bangkok",
  );

  // …and the two defective lines are gone from the executed source, not merely
  // shadowed by the imports above.
  assert.equal(
    /new Date\(`\$\{v\}T00:00:00`\)/.test(code),
    false,
    "the ambient-zone conversion is still in PublishDialog",
  );
  assert.equal(
    code.includes("String(v).slice(0, 10)"),
    false,
  ...
  assert.match(
    ...
    "the dialog stopped running the route’s predicate",
  );
});

test("CONTROL: the defective-line matchers do recognise their subjects", () => {
  assert.equal(
    /new Date\(`\$\{v\}T00:00:00`\)/.test(
      "const f = (v) => new Date(`${v}T00:00:00`).toISOString();",
    ),
    true,
    "the ambient-zone matcher does not work, so the check above means nothing",
  );
  assert.equal(
    "const g = (v) => String(v).slice(0, 10);".includes(
      "String(v).slice(0, 10)",
    ),
    true,
...
test("the field hints tell the author which day and whose clock", () => {
  ...
  assert.ok(
    code.includes("เข้าได้ถึงสิ้นวันนั้น (เวลาไทย)"),
    "the end-date field no longer says the named day is included",
  );
  assert.ok(
    code.includes("เริ่มตั้งแต่ต้นวันนั้น (เวลาไทย)"),
