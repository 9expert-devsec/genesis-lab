...
const hintFor = (doc, label) => {
  const l = [...doc.querySelectorAll("label")].find(
    (n) => text(n.querySelector(":scope > span")) === label,
  );
...
const optionValues = (doc, label) => {
  ...
  return [...(l?.querySelectorAll("option") ?? [])].map((o) =>
...
test("the two colour controls sit together under ONE group named for them", () => {
  ...
  assert.equal(
    groupsIn(tab()).includes("สไตล์"),
    false,
    "the one-child สไตล์ group is back",
  );
  assert.equal(
    groupsIn(tab()).includes("พื้นหลังและการแสดงผล"),
    false,
...
test("the accent control is RENAMED and its scope is unchanged", () => {
  ...
  assert.equal(
    fieldsIn(tab()).includes("สีเน้น"),
    false,
    "the old name is still rendered",
  );
  assert.equal(
    fieldsIn(tab()).includes("สีปุ่มกด"),
    false,
...
test("the background select offers every preset plus กำหนดเอง, and nothing else", () => {
  assert.deepEqual(optionValues(tab(), "พื้นหลัง"), [
    ...OFFERED_BACKGROUNDS,
    CUSTOM_COLOR_OPTION,
  ]);
  ...
  assert.equal(
    OFFERED_BACKGROUNDS.includes(CUSTOM_COLOR_OPTION),
    false,
...
test("the accent select keeps its ตามธีม sentinel and gains กำหนดเอง", () => {
  assert.deepEqual(optionValues(tab(), "สีองค์ประกอบ"), [
    "",
    ...ACCENTS,
    CUSTOM_COLOR_OPTION,
  ]);
...
test("the DIRECTION control appears only with a second stop", () => {
  ...
  const twoStops = tab({
    settings: {
      backgroundMode: "custom",
      backgroundCustom: { from: "#3366cc", to: "#cc6633" },
    },
  });
...
test("an INVALID second stop does not offer the direction either", () => {
  ...
  const half = tab({
    settings: {
      backgroundMode: "custom",
      backgroundCustom: { from: "#3366cc", to: "#cc66" },
    },
  });
...
test("the custom-mode caveat is on BOTH colour controls, by exact text", () => {
  ...
  assert.equal(
    hintFor(tab({ settings: CUSTOM_BG }), "สีเริ่มต้น"),
    CUSTOM_COLOR_CAVEAT,
  );
  assert.equal(
    hintFor(tab({ style: CUSTOM_ACCENT }), "สีที่กำหนดเอง"),
    CUSTOM_COLOR_CAVEAT,
  );
});
...
test("the caveat claims nothing about what a PRESET would have done", () => {
  ...
  for (const word of ["ต่างจาก", "แทนที่จะ", "ตามธีม จะ"]) {
    assert.equal(
      CUSTOM_COLOR_CAVEAT.includes(word),
      false,
  ...
  assert.equal(
    /ไม่ปรับสีนี้ตามธีมหรือโหมดมืด/.test(CUSTOM_COLOR_CAVEAT),
    false,
...
test("the contrast warnings fire on a bad colour and stay quiet on a good one", () => {
  ...
  const badBg = tab({
    settings: {
      backgroundMode: "custom",
      backgroundCustom: { from: "#808080" },
    },
  });
  assert.deepEqual(alerts(badBg), [BACKGROUND_CONTRAST_WARNING]);
  assert.deepEqual(
    alerts(tab({ settings: CUSTOM_BG })),
    [],
  ...
  const badAccent = tab({
    style: { accentMode: "custom", accentCustom: "#ffff00" },
  });
...
test("the warning WARNS — it changes no value and blocks nothing", () => {
  ...
  assert.equal(
    doc.querySelectorAll("input[disabled]").length,
    0,
    "the warning disabled the control",
  );
});
...
test("an EMPTY custom colour warns about nothing", () => {
  ...
  assert.deepEqual(
    alerts(
      tab({ settings: { backgroundMode: "custom", backgroundCustom: {} } }),
    ),
    [],
  );
...
test("the hint is UNCHANGED in custom mode — the scope did not move", () => {
   ...
   */
  assert.equal(
    hintFor(tab({ style: CUSTOM_ACCENT }), "สีองค์ประกอบ"),
    ACCENT_HINT,
  );
});
...
test("an untouched picker opens on the fallback, and the box stays EMPTY", () => {
  ...
  assert.equal(
    doc.querySelector('input[type="color"]').getAttribute("value"),
    COLOR_INPUT_FALLBACK,
  );
  assert.equal(
    doc.querySelector('input[type="text"]').getAttribute("value"),
    "",
  );
});
...
test("a half-typed hex is marked invalid, kept as typed, and not corrected", () => {
  ...
  const doc = domOf(
    createElement(ColorInput, { value: "#33", onChange: noop, invalid: true }),
  );
  assert.equal(
    doc.querySelector('input[type="text"]').getAttribute("value"),
    "#33",
  ...
  assert.equal(
    doc.querySelector('input[type="text"]').getAttribute("aria-invalid"),
    "true",
  );
...
test("the panel marks a bad value invalid, and leaves an empty one alone", () => {
  const bad = tab({
    settings: { backgroundMode: "custom", backgroundCustom: { from: "#33" } },
  });
  assert.equal(bad.querySelector('input[aria-invalid="true"]') !== null, true);
  const empty = tab({
    settings: { backgroundMode: "custom", backgroundCustom: {} },
  });
  assert.equal(
    empty.querySelector('input[aria-invalid="true"]'),
    null,
