...
import { SectionRenderer, RENDERABLE_SECTION_TYPES } from '@/components/pageBuilder/SectionRenderer';
...
const html = (section) => renderToStaticMarkup(createElement(SectionRenderer, { section }));
const docOf = (markup) => new JSDOM(`<!doctype html><body>${markup}</body>`).window.document;
const wrapperOf = (section) => docOf(html(section)).querySelector('section');

const sec = (over = {}) => ({
  id: 's1', type: 'heading', name: '', enabled: true, sortOrder: 0,
...
test('a section with a PRESET background renders the class and no inline style', () => {
  for (const bg of ['default', 'white', 'light', 'soft_gray', 'dark', 'brand_gradient']) {
    const el = wrapperOf(sec({ settings: { background: bg } }));
    const cls = backgroundClass(bg);
    if (cls) assert.ok(el.className.split(/\s+/).includes(cls), `${bg} lost its class`);
...
test('a section with NO colour choice emits no style attribute at all', () => {
  ...
  assert.equal(wrapperOf(sec({ settings: { background: 'default' } })).getAttribute('style'), null);
});
...
test('the preset background CLASS is gone when a custom colour takes over', () => {
   ...
   */
  const el = wrapperOf(customBg({ from: '#123456' }));
  assert.equal(el.className.split(/\s+/).includes(backgroundClass('dark')), false,
  ...
  assert.equal(el.className.includes('text-9e-ice'), false,
...
test('an INVALID custom value falls back to the preset, rendered', () => {
  for (const bad of ['#abc', 'rgb(1,2,3)', '#123456; color:red', 'navy', '']) {
    ...
    assert.equal(el.getAttribute('style'), null, `"${bad}" reached the style attribute`);
    assert.ok(el.className.split(/\s+/).includes(backgroundClass('dark')),
...
function accentPaintingTypes() {
  const files = readdirSync(new URL(`../../${SECTIONS_DIR}/`, import.meta.url))
...
const CONTENT_FOR = {
  ...
  cta: { heading: 'หัวข้อ', description: 'คำอธิบาย', buttonLabel: 'กดเลย', buttonHref: '/x' },
  rich_text: { doc: { type: 'doc', content: [{ type: 'paragraph', content: [{ type: 'text', text: 'ข้อความ' }] }] } },
  icon_card: { icon: 'Star', title: 'การ์ด', description: 'เนื้อหา' },
  stat_card: { value: '99', label: 'สถิติ' },
  price_card: { title: 'แพ็กเกจ', price: '1,900', features: ['หนึ่ง'], buttonLabel: 'ซื้อ', buttonHref: '/x' },
  ...
  instructor_card: { name: 'ผู้สอน', title: 'อาจารย์', bio: 'ประวัติ', specialties: ['Data'] },
  course_schedule: {},
};
...
const PAINTS_FROM_FIXTURE = [
  'checklist', 'cta', 'icon_card', 'price_card', 'rich_text', 'stat_card', 'timeline',
];
...
test('a CUSTOM accent reaches every one of those eleven — and only by the same route', () => {
   ...
   */
  const refs = (section) => [...docOf(html(section)).querySelectorAll('*')]
  ...
  for (const type of RENDERABLE_SECTION_TYPES) {
    ...
    const custom = refs(sec({ ...base, style: { accentMode: 'custom', accentCustom: '#123456' } }));
   ...
   */
  assert.deepEqual(painted.sort(), PAINTS_FROM_FIXTURE,
...
test('the custom accent sets the same three variables, with the author\'s value', () => {
  const el = wrapperOf(sec({ style: { accentMode: 'custom', accentCustom: '#123456' } }));
  const map = styleMap(el);
  assert.deepEqual(Object.keys(map).sort(), ['--pb-accent-fill', '--pb-accent-on', '--pb-accent-text']);
  ...
  assert.deepEqual(Object.keys(map).sort(), Object.keys(accentVars('green')).sort());
});

test('a custom accent and a custom background share ONE style attribute', () => {
  const el = wrapperOf(sec({
    settings: { backgroundMode: 'custom', backgroundCustom: { from: '#123456' } },
...
test('a CUSTOM accent cascades into children — by containment, not by class', () => {
   ...
   */
  const parent = sec({
    id: 'outer', type: 'container',
    style: { accentMode: 'custom', accentCustom: '#123456' },
    content: { children: [sec({ id: 'inner', type: 'checklist', content: { items: [{ text: 'หนึ่ง', checked: true }, { text: 'สอง', checked: true }] } })] },
  });
  const doc = docOf(html(parent));

  const declaring = [...doc.querySelectorAll('section')]
    .find((n) => (n.getAttribute('style') ?? '').includes('--pb-accent-fill'));
  assert.notEqual(declaring, undefined, 'no element declared the custom accent');
  assert.equal(styleMap(declaring)['--pb-accent-fill'], '#123456');

  const painting = [...doc.querySelectorAll('*')]
    .filter((n) => /--pb-accent-/.test(n.getAttribute('class') ?? ''));
  assert.ok(painting.length > 0, 'the nested child painted with no accent at all');
  for (const n of painting) {
    assert.ok(declaring.contains(n) && declaring !== n,
  ...
  if (inner) {
    assert.equal((inner.getAttribute('style') ?? '').includes('--pb-accent-fill'), false,
...
test('a PRESET accent cascades by the identical mechanism — same containment', () => {
  ...
  const build = (style) => sec({
    id: 'outer', type: 'container', style,
    content: { children: [sec({ id: 'inner', type: 'checklist', content: { items: [{ text: 'หนึ่ง', checked: true }, { text: 'สอง', checked: true }] } })] },
  });
  const shape = (style) => {
  ...
  assert.ok(shape({ accentColor: 'green' }).painting.length > 0,
    'the preset side painted nothing, so the comparison above compares two empties');
  assert.ok(shape({ accentMode: 'custom', accentCustom: '#123456' }).painting.length > 0);
});
...
test('CONTROL: a section with NO accent leaves the children inheriting the page', () => {
  ...
  const doc = docOf(html(sec({
    id: 'outer', type: 'container',
    content: { children: [sec({ id: 'inner', type: 'checklist', content: { items: [{ text: 'หนึ่ง', checked: true }] } })] },
  })));
  const declaring = [...doc.querySelectorAll('section')]
    .filter((n) => (n.getAttribute('style') ?? '').includes('--pb-accent-fill'));
  assert.deepEqual(declaring, [], 'a section that set no accent declared one anyway');
});
...
test('the renderer cannot reach the mode-blind resolvers', () => {
   ...
   */
  const { withImports } = readSource('src/components/pageBuilder/SectionRenderer.jsx');
  for (const raw of ['backgroundClass,', 'isDarkBackground,', 'accentVars,']) {
    assert.equal(withImports.includes(raw), false,
      `SectionRenderer imports the mode-blind "${raw.slice(0, -1)}" again — there must be one path`);
  }
  for (const wrapped of ['backgroundClassFor', 'backgroundStyleFor', 'isDarkBackgroundFor', 'accentVarsFor']) {
    assert.ok(withImports.includes(wrapped), `SectionRenderer no longer uses ${wrapped}`);
  }
});

test('CONTROL: that import check DOES see a mode-blind name when one is there', () => {
  const planted = "import { backgroundClass, spacingTopClass } from '@/lib/pageBuilder/presets';";
  assert.equal(planted.includes('backgroundClass,'), true);
  // …and it is not matching the wrapper by accident.
  assert.equal("import { backgroundClassFor } from 'x';".includes('backgroundClass,'), false);
});
