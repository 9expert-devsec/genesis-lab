 ...
 */
const offered = (types) =>
  [...types].filter((t) => !RETIRED_SECTION_TYPES.includes(t));
...
function bodyDoc({
  ...
  const markup = renderToStaticMarkup(
    createElement(SectionPickerBody, {
      query,
      activeGroup,
      canUseAdvanced,
      onQueryChange: () => {},
...
const groupKeysIn = (doc) =>
  [...doc.querySelectorAll('[data-testid="picker-group"]')].map((g) =>
    g.getAttribute("data-group-key"),
  );
...
test("CONTROL: an unfiltered render draws all five groups and all 27 types", () => {
  ...
  assert.equal(
    doc.querySelector('[data-testid="picker-empty"]'),
    null,
...
test("a query matching ONE label leaves exactly that type, in exactly its own group", () => {
  ...
  assert.equal(
    text(doc.querySelector('[data-testid="picker-type"]')),
    labelOf("timeline"),
  );
});
...
test("a query matching a shared Thai substring returns EVERY label carrying it, across groups", () => {
  ...
  assert.deepEqual(
    groupKeysIn(doc),
    ["layout", "card"],
    "the groups are drawn out of GROUPS order",
  );
...
test("a query matching NOTHING draws no groups at all — not empty headers", () => {
  ...
  assert.deepEqual(
    groupKeysIn(doc),
    [],
    "a group header rendered with no types under it",
  );
  assert.deepEqual(typesIn(doc), []);
  // …and the author is told so, rather than being shown a blank dialog.
  assert.equal(
    text(doc.querySelector('[data-testid="picker-empty"]')),
...
test("CONTROL: the empty case is reachable ONLY through the filter, not by rendering nothing", () => {
  ...
  assert.equal(
    groupKeysIn(bodyDoc({ query: "zzzzไม่มีอะไรตรงzzzz" })).length,
    0,
  );
...
test("each pill narrows to exactly its own exported list", () => {
  ...
  for (const group of GROUPS) {
    ...
    assert.deepEqual(
      groupKeysIn(doc),
      [group.key],
      `the ${group.key} pill drew another group too`,
    );
    assert.deepEqual(
      typesIn(doc),
      offered(expected[group.key]).sort(),
...
test("ทั้งหมด shows every group again", () => {
  assert.deepEqual(
    groupKeysIn(bodyDoc({ activeGroup: "all" })),
    GROUPS.map((g) => g.key),
  );
});
...
test("the active pill carries this codebase’s selected-state treatment, and exactly one does", () => {
  ...
  assert.equal(
    active.length,
    1,
    "more than one pill, or none, read as selected",
  );
  ...
  assert.ok(
    cls.includes("border-9e-action/40"),
    "the active pill lost the shared selected border",
  );
  assert.ok(
    cls.includes("bg-9e-action/10"),
    "the active pill lost the shared selected fill",
  );
  ...
  assert.equal(
    inactive.getAttribute("class").includes("bg-9e-action/10"),
    false,
  );
});
...
test("a query matching a type OUTSIDE the active group hides it", () => {
  ...
  assert.equal(
    typesIn(both).includes("card_grid"),
    false,
...
test("CONTROL: that same query and that same pill each show something on their own", () => {
  ...
  assert.deepEqual(
    typesIn(bodyDoc({ query: "ไทม์ไลน์", activeGroup: "all" })),
    ["timeline"],
  );
  assert.deepEqual(
    typesIn(bodyDoc({ query: "", activeGroup: "card" })),
    [...CARD_TYPES].sort(),
  );
});
...
test("a NON-developer sees one locked summary row and NOT ONE advanced button", () => {
  ...
  assert.ok(
    summary,
    "the collapsed Advanced row is missing for a non-developer",
  );
  assert.ok(
    text(summary).startsWith("ต้องมีสิทธิ์ developer"),
  ...
  for (const type of ADVANCED_TYPES) {
    assert.equal(
      drawn.has(type),
      false,
      `${type} still rendered as its own button for a non-developer`,
    );
  }
...
test("a DEVELOPER sees every advanced type as its own button, in the same grid as everything else", () => {
  ...
  assert.equal(
    doc.querySelector('[data-testid="picker-advanced-locked"]'),
    null,
  ...
  assert.deepEqual(
    [...new Set(advanced.map((b) => b.getAttribute("data-state")))],
    ["add"],
  );
});
...
test("CONTROL: the two tiers differ ONLY in the advanced group", () => {
  ...
  const dev = typesIn(bodyDoc({ canUseAdvanced: DEV })).filter(
    (t) => !ADVANCED_TYPES.includes(t),
  );
  const nonDev = typesIn(bodyDoc({ canUseAdvanced: NON_DEV })).filter(
    (t) => !ADVANCED_TYPES.includes(t),
  );
  assert.deepEqual(dev, nonDev);
  assert.ok(
    dev.length > 0,
    "both tiers drew nothing, so the comparison is vacuous",
  );
});
...
test('every button the new layout draws is disabled iff its computed state is not "add"', () => {
  ...
  assert.ok(
    buttons.length > 0,
    "no buttons drawn — the check below is vacuous",
  );
...
test('a container type in "add" state still says it can hold nested sections', () => {
  ...
  for (const b of containers) {
    assert.equal(
      ...
      "add",
      "a container type is no longer offerable",
    );
    assert.ok(
      b.textContent.includes("ใส่ section ซ้อนข้างในได้"),
...
test('the rendered pill row is exactly "ทั้งหมด" plus one pill per GROUPS entry, in order', () => {
  const doc = bodyDoc();
  const keys = [...doc.querySelectorAll('[data-testid="picker-pill"]')].map(
    (p) => p.getAttribute("data-group-key"),
  );
  assert.deepEqual(keys, ["all", ...GROUPS.map((g) => g.key)]);
  const titles = [...doc.querySelectorAll('[data-testid="picker-pill"]')].map(
    text,
  );
...
test("visibleGroups drops narrowed-to-nothing groups rather than emptying them", () => {
  ...
  assert.deepEqual(
    groups[0].types,
    ["card_grid"],
    "the layout group was passed through unnarrowed",
  );
