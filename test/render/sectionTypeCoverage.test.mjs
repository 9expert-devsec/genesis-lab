...
test('every declared section type has a component — so the picker’s "soon" state is unreachable TODAY', () => {
  const orphans = declaredButUndrawable(
    ALL_SECTION_TYPES,
...
test("CONTROL: the same function reddens on a type list that HAS an undrawable type", () => {
  ...
  assert.deepEqual(
    declaredButUndrawable(fakeDeclared, RENDERABLE_SECTION_TYPES),
    ["pull_quote"],
  );
  ...
  assert.deepEqual(
    declaredButUndrawable(["heading"], RENDERABLE_SECTION_TYPES),
    [],
  );
  assert.deepEqual(
    declaredButUndrawable(["heading", "pull_quote"], RENDERABLE_SECTION_TYPES),
    ["pull_quote"],
  );
});
...
test("the picker displays the five category lists, which is what ALL_SECTION_TYPES concatenates", () => {
  ...
  const picker = readSource(
    "src/components/pageBuilder/editor/SectionPicker.jsx",
  ).code;
  for (const list of [
    "LAYOUT_TYPES",
    "CONTENT_TYPES",
...
const pickerDoc = (canUseAdvanced) =>
  new JSDOM(
...
test("CONTROL: the two tiers really do differ, so neither assertion above is vacuous", () => {
  ...
  assert.ok(
    dev.length > 0 && nonDev.length > 0,
    "a tier drew nothing clickable at all",
  );
  assert.notDeepEqual(dev, nonDev);
  assert.deepEqual(
    dev.filter((t) => !nonDev.includes(t)).sort(),
    [...ADVANCED_TYPES].sort(),
