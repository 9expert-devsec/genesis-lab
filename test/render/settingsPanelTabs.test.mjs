...
const fieldsIn = (doc) =>
  [...doc.querySelectorAll("label > span:first-child")].map((s) =>
    s.textContent.trim(),
  );
const groupsIn = (doc) =>
  [...doc.querySelectorAll("legend")].map((l) => l.textContent.trim());
...
const styleTab = (type, { layout = {}, style = {}, settings = {} } = {}) =>
  domOf(
    createElement(StyleTab, {
      type,
      layout,
      style,
      settings,
...
const advancedTab = (advanced, canUseAdvanced) =>
  domOf(
    createElement(AdvancedGroup, {
      path: ["sections", 0],
      advanced,
      canUseAdvanced,
      dispatch: noop,
...
test("the เนื้อหา tab renders exactly the per-type content fields and nothing from รูปแบบ", () => {
  ...
  for (const f of [...ENVELOPE_FIELDS, ...ADVANCED_FIELDS]) {
    assert.equal(
      fieldsIn(doc).includes(f),
      false,
      `"${f}" leaked into the content tab`,
    );
  }
});
...
test("the ขั้นสูง tab renders exactly the four developer fields", () => {
  ...
  for (const f of [...ENVELOPE_FIELDS, "ข้อความ"]) {
    assert.equal(
      fieldsIn(doc).includes(f),
      false,
      `"${f}" leaked into the advanced tab`,
    );
  }
});
...
test("round 17 added a field to the panel and did NOT change the expected union", () => {
  ...
  assert.deepEqual(
    fieldsIn(nameDoc),
    ["ชื่อเรียกภายใน"],
    "the name field changed its label",
  );
  ...
  for (const type of [
    ...
    for (const [where, doc] of [
      ["เนื้อหา", contentTab(type)],
      ["รูปแบบ", styleTab(type)],
    ]) {
      assert.equal(
        fieldsIn(doc).includes("ชื่อเรียกภายใน"),
        false,
  ...
  assert.equal(
    fieldsIn(advancedTab({}, true)).includes("ชื่อเรียกภายใน"),
    false,
  );
});
...
test("CONTROL: the union WOULD have needed extending had the field gone in a tab", () => {
  ...
  assert.throws(
    () => assert.deepEqual(fieldsIn(withField), ENVELOPE_FIELDS),
...
test("the panel dispatches the TOP-LEVEL merge for the name, not the sub-object one", () => {
  ...
  const call = code.slice(
    code.indexOf("<SectionNameField"),
    code.indexOf("<Tabs.Root"),
  );
...
test("CONTROL: the union check is sensitive to a single missing field, by name", () => {
  ...
  assert.deepEqual(
    ENVELOPE_FIELDS.filter((f) => !missing.includes(f)),
    ["แสดงบน"],
  );
});
...
test("the tab strip does not re-implement the rule — it calls the one function", () => {
  ...
  assert.equal(
    code.split("hasAdvancedTab(").length - 1,
    3,
...
test("แสดงบน is still a Select over all four VISIBILITY options, not a toggle", () => {
  ...
  const visibility = selects.find((s) =>
    [...s.options].some((o) => o.value === "hidden"),
  );
  assert.ok(visibility, "no select carries the visibility vocabulary any more");
  assert.deepEqual(
    [...visibility.options].map((o) => o.value),
    [...VISIBILITY],
  );
  assert.equal(
    VISIBILITY.length,
    4,
    "the vocabulary itself changed size — re-check this round",
  );
  assert.deepEqual(
...
test("CONTROL: the visibility probe WOULD catch a two-state replacement", () => {
  ...
  const doc = styleTab("heading");
  const visibility = [...doc.querySelectorAll("select")].find((s) =>
    [...s.options].some((o) => o.value === "hidden"),
  );
  assert.deepEqual(
    [...visibility.options].map((o) => o.value),
...
test("the active tab is DERIVED by clamping to the tabs that exist, never by an effect", () => {
  ...
  assert.equal(
    /dispatch\(\{ type: '[A-Z_]*TAB/.test(code),
    false,
...
test("each Tabs.Content holds the body tested above, and the empty state has no strip", () => {
  ...
  for (const [value, body] of [
    ...
    assert.ok(
      code.slice(at, end).includes(`<${body}`),
      `the ${value} tab does not hold ${body}`,
    );
  }
  ...
  assert.ok(
    empty < code.indexOf("<Tabs.Root"),
    "the empty state no longer returns before the tab strip",
  );
});
...
test("Radix Tabs renders INLINE, not through a portal — verified, not assumed", () => {
  ...
  assert.ok(
    markup.includes("TRIGGER_A"),
    "Tabs.Trigger produced no server markup",
  );
  assert.ok(
    markup.includes("PANEL_A"),
    "the ACTIVE Tabs.Content produced no server markup",
  );
  ...
  assert.equal(
    markup.includes("PANEL_B"),
    false,
    "the inactive panel also rendered",
  );
});
...
test("the header names the selected type, once", () => {
  const doc = domOf(
    createElement(SelectionHeader, { type: "heading", parentType: null }),
  );
  assert.equal(
    text(doc.querySelector('[data-testid="settings-header-type"]')),
    "หัวข้อ",
  );
  ...
  const all = [
    ...doc.querySelectorAll('[data-testid="settings-header"] p'),
  ].map(text);
  assert.deepEqual(
    all,
    ["หัวข้อ"],
    "the header renders more than the type for a top-level section",
  );
});
...
test("a NESTED selection names the section it sits in", () => {
  ...
  assert.equal(
    text(doc.querySelector('[data-testid="settings-header-parent"]')),
    "อยู่ใน สองคอลัมน์",
  );
});
...
test('a TOP-LEVEL selection renders no parent line at all — never a dangling "อยู่ใน"', () => {
  ...
  assert.equal(
    doc.querySelector('[data-testid="settings-header-parent"]'),
    null,
  );
  assert.equal(
    text(doc.querySelector('[data-testid="settings-header"]')),
    "หัวข้อ",
  );
  assert.equal(
    /อยู่ใน/.test(text(doc.querySelector('[data-testid="settings-header"]'))),
    false,
...
test("CONTROL: the parent line is reachable, so its absence above means something", () => {
  ...
  assert.equal(
    without.querySelector('[data-testid="settings-header-parent"]'),
    null,
  );
  ...
  const blank = domOf(
    createElement(SelectionHeader, { type: "heading", parentType: "" }),
  );
  assert.equal(
    blank.querySelector('[data-testid="settings-header-parent"]'),
    null,
  );
});
...
test("the type is not stated twice anywhere in the panel body", () => {
  ...
  assert.equal(
    code.split("labelOf(selected.type)").length - 1,
    0,
...
test("the content tab no longer repeats the tab label as a legend", () => {
  ...
  assert.deepEqual(groupsIn(styleTab("two_column")), [
    "เลย์เอาต์",
    ...ENVELOPE_GROUPS,
  ]);
});
...
const fieldPairsIn = (doc) =>
  [...doc.querySelectorAll("label")].map((l) => {
    const spans = [...l.querySelectorAll(":scope > span")];
    return {
      label: text(spans[0]),
      hint: spans.length > 1 ? text(spans[1]) : null,
    };
  });

const hintFor = (doc, label) =>
  fieldPairsIn(doc).find((f) => f.label === label)?.hint ?? null;
...
test("CONTROL: the hint reader sees hints at all, and tells them from labels", () => {
   ...
   */
  const withHint = domOf(
    createElement(Field, { label: "ป้าย", hint: "คำอธิบาย" }, "x"),
  );
  assert.deepEqual(fieldPairsIn(withHint), [
    { label: "ป้าย", hint: "คำอธิบาย" },
  ]);
  ...
  const withOld = domOf(
    createElement(
      Field,
      { label: "สีเน้น", hint: OLD_UNIVERSAL_ACCENT_HINT },
      "x",
    ),
  );
...
test("A: the universal accent claim is gone, and the replacement is exactly this", () => {
  ...
  for (const type of [
    ...
    assert.equal(
      hintFor(styleTab(type), ACCENT_FIELD_LABEL),
      ROUND22_ACCENT_HINT,
  ...
  assert.equal(
    rendered.includes(OLD_UNIVERSAL_ACCENT_HINT),
    false,
  ...
  assert.equal(
    readSource(SRC).raw.includes(OLD_UNIVERSAL_ACCENT_HINT),
    false,
...
test("A: the accent hint is ONE string — it does not vary by type", () => {
   ...
   */
  const hints = new Set(
    ALL_SECTION_TYPES.map((t) => hintFor(styleTab(t), ACCENT_FIELD_LABEL)),
  );
...
test("B: ความกว้าง is still offered on every one of the 27 types", () => {
   ...
   */
  const offering = ALL_SECTION_TYPES.filter((t) =>
...
test("B: exactly the two self-clamping types say their card width is fixed", () => {
  const hinted = ALL_SECTION_TYPES.filter(
    (t) => hintFor(styleTab(t), "ความกว้าง") === ROUND22_WIDTH_HINT,
  ).sort();
  ...
  assert.deepEqual(
    [...new Set(others.map((t) => hintFor(styleTab(t), "ความกว้าง")))],
    [null],
  );
});
...
test("CONTROL: the one-source check catches the two sets disagreeing", () => {
  ...
  assert.throws(
    () => assert.deepEqual([...base, "price_card"].sort(), base),
    "a third clamped type does not break the comparison",
  );
  assert.throws(
    () => assert.deepEqual(["course_card"], base),
    "a type losing its clamp does not break the comparison",
  );
  assert.deepEqual(
    [...base].sort(),
    base,
    "the unperturbed comparison must pass",
  );
});
...
test("C: the panel comment corrects the two fields this round made true, and stops there", () => {
  ...
  assert.equal(
    code.includes("EXACT FOR THREE OF THE FIVE"),
    false,
  ...
  assert.match(
    head,
    /EXACT FOR THREE OF THE FIVE/,
    "the measured qualification is gone",
  );
  ...
  assert.match(
    head,
    /wrap themselves in a small fixed/,
    "the comment no longer names the clamp that limits containerWidth",
  );
  assert.match(
    head,
    /painted card is 384px at all four settings/,
    "the comment no longer says what the clamp costs the author",
  );
  assert.match(
    head,
    /universal as a CASCADE/,
    "the comment no longer separates cascade from effect",
  );
  ...
  assert.match(
    head,
    /accent gap in the renderers is closed/,
    "the comment no longer states the accent gap as closed",
  );
  assert.equal(
    /DO have a surface the accent belongs on and\s*\n\s*\*\s*do not take it/.test(
      head,
    ),
    false,
