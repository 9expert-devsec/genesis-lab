...
// 1489 (two delivery fixes: gif skip-transform, mp3 no-store) — +4 in
//   test/pure/legacyDerivativeRewrite.test.mjs, +4 appended to legacyBlobRouting.test.mjs.
//   THE GIF FIX came from production, not from reading code. Two 1920x1080 animated GIFs (119 and
//   54 frames) returned HTTP 400 with x-cld-error "Maximum total number of pixels in all
//   frames/pages is 50 Megapixels". The ASSETS were always fine — untransformed delivery is 200
//   byte-equal to source — it is the TRANSFORM that Cloudinary refuses, and the layer applied it to
//   every image. Frame count is not in a path, so no rewrite could single those two out; excluding
//   the whole extension is the only rule a pattern can express, which is why svg's special case
//   became UNTRANSFORMED_EXTENSIONS. The cost (27 other GIFs now ship original bytes rather than a
//   smaller webp — ~1.9 MB more on the largest) is stated in that constant and bounded by a CONTROL
//   that .png still carries the default transformation, so the exclusion cannot quietly widen and
//   take the whole fixed-transformation bandwidth argument with it.
//   THE TEST-DESIGN LESSON, which is the part worth re-reading: three of these assertions first
//   derived their expected extension set FROM UNTRANSFORMED_EXTENSIONS, so deleting 'gif' from the
//   constant left them green — they had quietly become "whatever the constant says, the rules agree
//   with it", which is true by construction and worth nothing. The set is pinned LITERALLY now and
//   the loop iterates literal extensions; the same break then reddens 3 tests instead of 1.
//   ALSO: selecting the untransformed rule by "source contains gif and svg" additionally matched the
//   DERIVATIVE rule, whose pattern embeds the entire IMAGE_EXTENSIONS alternation and legitimately
//   DOES transform. The selector is the composed alternation string now.
//   mp3 joins NO_STORE_DOCUMENT_EXTENSIONS and DELIBERATELY NOT RAW_EXTENSION_LIST. An <audio>
//   element seeks constantly, so it needs the same 206-on-Range guarantee a PDF viewer does — and
//   the same protection from a cache answering 200 with a partial body, which makes a player treat
//   a 30 MB podcast as fully buffered. It is not in RAW_EXTENSION_LIST because that list means
//   "Cloudinary serves this as raw", and these five are not on Cloudinary at all; adding it there
//   would emit a Cloudinary rule that silently claims these paths the moment rule order changed.
// 1481 (Vercel Blob track) — +10 in the new test/pure/legacyBlobRouting.test.mjs. 19 legacy files
//   cannot live on Cloudinary: its raw ceiling is 10 MB on any plan this project would consider and
//   one of them is 42.6 MiB. Reaching them is STRUCTURALLY different from everything else in the
//   delivery layer — Cloudinary's public_id IS the path, so a handful of pattern rules serve ~2,900
//   files with no lookup, while a blob pathname is whatever it was named, so it is ONE REWRITE PER
//   FILE off a manifest GENERATED by the upload run. A hand-kept list of 16 paths and a store
//   holding 16 objects drift, and the drift is a 404 for a file that uploaded perfectly well.
//   THE FAILURE THESE GUARD: /files and /images are LEGACY_ROOTS, so the Cloudinary catch-all
//   matches every blob path too. First match wins, so a blob rule emitted AFTER the catch-all sends
//   the request to Cloudinary — where the large files are absent and mp3 is in neither extension
//   set. Nothing throws; the URL just 404s. Order is the whole mechanism, so it is asserted
//   directly and per-file against the first Cloudinary rule. Verified red by moving the blob spread
//   below the catch-alls: 4 tests fail.
//   ONE CONTROL LESSON WORTH KEEPING: the first attempt at that control changed nothing, and the
//   tempting reading was "the ordering tests are weak". They were not — the patch had anchored on
//   \n against a CRLF file and silently duplicated the spread instead of moving it. A green control
//   here means "check the patch applied" BEFORE "check the test".
//   Inertness without BLOB_PUBLIC_BASE is a TESTED property, not an accident: rewrites() reads the
//   env at call time, so the file calls it both ways, including the control that an mp3 then falls
//   through to Cloudinary — which is exactly why it 404s until the env is set and the site
//   redeployed. The manifest set is pinned at 16 with 5 mp3, and no entry may sit at the site root,
//   because that is where every application page lives and why the 3 webroot PDFs keep EXPLICIT
//   hand-written rules instead of joining the generated list.
